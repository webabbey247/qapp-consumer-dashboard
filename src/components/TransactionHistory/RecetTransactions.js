...
const RecetTransactions = () => {
  const [recetTransaction, setRecentTransaction] = React.useState('');  
  
  const recentSpending = () => {
    apiOperation.get("account/transactions/0").then((res) => {
      if(res.data.success === false) {
      } else {   
        setRecentTransaction(res.data.result.transactions)
      console.log("The total spending", );
      }
  });
  }
  
  React.useEffect(() => recentSpending());
...
export default RecetTransactions;
